/* CATENARY.C   M. W. Klotz   6/09

Compute parameters for catenary
...
{
int i;
dbl tension;	//tension in cable (lbf)
dbl density;	//cable weight per unit length (lb/ft)
dbl distance;	//straightline distance between supports (ft)
dbl param;	//parameter (ft)
dbl droop;	//droop at center of cable (ft)
dbl length;	//cable length (ft)
dbl x;
...
vin ("Tension in cable",&tension,"lf",0.674427074,"lbf");
vin ("Cable weight per unit length",&density,"lf",0.00044091,"lb/ft");
vin ("Straightline distance between supports",&distance,"lf",1.640419948,"ft");

param=tension/density;
x=.5*distance;
droop=param*(cosh(x/param)-1.);
length=2*param*sinh(0.5*distance/param);

print;
printf ("Cable droop at center of span = %.8g ft\n",droop);
printf ("Cable length = %.10g ft\n",length);
