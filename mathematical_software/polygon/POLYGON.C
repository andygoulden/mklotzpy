/* POLYGON.C   M. W. Klotz  01/01, 08/03

Various properties of regular polygons
...
{
int i,j,k,n,even;
dbl ang,daf,dfv,r,f,c,s,p,area,dc,di,ca,sa;

/* -------------------------- initializations ------------------------------ */
textscreen ();
/* ------------------------------------------------------------------------- */

puts ("PROPERTIES OF REGULAR POLYGONS\n");

agin: vin ("Number of polygon sides",&n,"d",6.,"");
if (n<3)
{
beep;
...
even=!fmod((dbl)n,2.);
ang=360./n;
ca=COSD(0.5*ang); sa=SIND(0.5*ang);

puts ("\nYou must answer at least one of the following four questions.");
puts ("Press enter if you don't know quantity requested.\n");

vin ("Length of side",&c,"lf",0.,"");
if (c) {r=0.5*c/sa; goto report;}

if (even)
{
vin ("Size across flats",&daf,"lf",0.,"");
if (daf) {r=daf/(2.*ca); goto report;}
}
else
{
vin ("Size flat-to-opposite-vertex",&dfv,"lf",0.,"");
if (dfv) {r=dfv/(1.+ca); goto report;}
}

vin ("Diameter of circumscribed circle",&dc,"lf",0.,"");
if (dc) {r=0.5*dc; goto report;}

vin ("Diameter of inscribed circle",&di,"lf",0.,"");
if (di) {f=0.5*di; r=f/ca; goto report;}

beep;
puts ("\nINSUFFICIENT INFORMATION FOR SOLUTION");
goto mexit;
...
area=0.5*c*f*n;
printf ("\nNumber of sides = %d\n",n);
printf ("Central angle between vertices = %.4lf deg\n",ang);
printf ("Vertex angle = %.4lf deg\n",180.-ang);
printf ("Diameter of circumscribed circle = %.4lf\n",2.*r);
printf ("Diameter of inscribed circle = %.4lf\n",2.*f);
if (even)
printf ("Distance across flats = %.4lf\n",2.*f);
else
printf ("Distance flat to opposite vertex = %.4lf\n",f+r);
printf ("Length of side = %.4lf\n",c);
