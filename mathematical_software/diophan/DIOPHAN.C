...
#define NQR 100		//maximum space for working variables
long q[NQR];		//working variable
long r[NQR];		//working variable
long s[NQR];		//working variable
...
{
int i,j,k;
long a,b,c,g,x,y,xp,yp,z;

/* -------------------------- initializations ------------------------------ */
textscreen ();
/* ------------------------------------------------------------------------- */

puts ("SOLUTION OF LINEAR DIOPHANTINE EQUATIONS\n");

puts ("Find integer x and y to satisfy the equation: ax + by = c");
puts ("where a,b, and c are integers.");
puts ("c must be evenly divisible by the gcd of a and b for a solution.");

print;

vin ("Integer value of a",&a,"ld",172.,"");
vin ("Integer value of b",&b,"ld",20.,"");
vin ("Integer value of c",&c,"ld",1000.,"");

print;

if (diophan (a,b,c,&x,&y) == 0) {puts ("NO SOLUTION WAS FOUND"); goto mexit;}

g=gcd(a,b);
printf ("x = %ld + k*(%ld), y = %ld - k*(%ld)  (k integer)\n",x,b/g,y,a/g);

puts ("\nSome sample solutions:");
for (k=-4 ; k<5 ; k++)
{
xp=x+k*b/g; yp=y-k*a/g; z=a*xp+b*yp;
printf ("k=%+2d: (%ld)*(%ld) + (%ld)*(%ld) = %ld\n",k,a,xp,b,yp,z);
}
...
Given a*x + b*y = c, where a,b,c are input integers, find INTEGER x and y
Returns:
	0 => no solution found
	1 => solution found

Uses working storage q[],r[],s[].

*/

int diophan (long a, long b, long c, long *x, long *y)
...
g=gcd(a,b);
if (fmod((dbl)c,(dbl)g) != 0)
{
printf ("gcd(%ld,%ld) = %ld\n",a,b,g);
printf ("c = %ld not evenly divisible by gcd\n",c);
puts ("EQUATION NOT SOLVABLE IN INTEGERS"); goto bailout;
}
...
do
{
i++; if (i>NQR) goto bailout;		// prevent runaway
k*=-1;
q[i]=r[i]/r[i+1]; 
r[i+2]=r[i]-q[i]*r[i+1];
} while (r[i+2]!=0);
...
s[i]=1; s[i+1]=0; i--;
do {s[i]=q[i]*s[i+1]+s[i+2]; i--; k*=-1;} while (i>0);

*x=k*s[2]*c/g; *y=-k*s[1]*c/g; return 1;
