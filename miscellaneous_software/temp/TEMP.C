/* TEMP.C   M. W. Klotz   9/99, 3/03, 1/06
...
dbl r0;			/* convert Fahrenheit to Rankine */
char scale;		/* input temperature scale */
dbl temp,tk,tc,tr,tf,te;	/* temperature variables */
...
r0=1.8*c0-32.;		/* so 0k = 0r else things get confusing */

if (argc > 1) {strcpy (buf, argv[1]); goto there;}

whoops:
puts ("Temperature scales are");
puts ("  Centigrade");
puts ("  Fahrenheit");
puts ("  Kelvin");
puts ("  Rankine");
...
agin:
puts ("Enter temperature as 123.45x, x=c,[f],k,r,e or q(uit))");
gets (buf);
there:
trim (buf); strlwr(buf);
if ((strstr(buf,"q")) != NULL) goto mexit;
temp=(dbl)atof(buf);
scale='f';
if ((strstr(buf,"c")) != NULL) scale='c';
if ((strstr(buf,"k")) != NULL) scale='k';
if ((strstr(buf,"r")) != NULL) scale='r';
if ((strstr(buf,"e")) != NULL) scale='e';

if (temp < 0. && (scale=='k' || scale=='r'))
{beep; puts ("Absolute temperatures are always positive!"); goto agin;}
...
print;
printf ("Centigrade = %10.3lf\n",tc);
printf ("Fahrenheit = %10.3lf\n",tf);
printf ("Kelvin     = %10.3lf\n",tk);
printf ("Rankine    = %10.3lf\n",tr);
printf ("Reaumur    = %10.3lf\n",te);
print;
goto agin;
