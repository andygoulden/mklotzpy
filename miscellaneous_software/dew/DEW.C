...
dbl a=17.27;
dbl b=237.7;		// degC
dbl T;			// measured temperature (degC)
dbl RH;			// relative humidity
dbl Td;			// dew point temperature (degC)
dbl alpha;
...
vin ("Measured temperature",&T,"lf",t,temp); if (fc == 'f') T=(T-32.)/1.8;

vin ("Relative humidity",&RH,"lf",60.,"%"); RH*=0.01;

alpha=a*T/(b+T)+log(RH);
Td=b*alpha/(a-alpha);		//dew point degC

printf ("Dew point temperature = %.2lf degC = %.2lf degF\n",Td,1.8*Td+32.);
t=0.04;
printf ("Accuracy = +/- %.2lf degC = +/- %.2lf degF\n",t,1.8*t);
