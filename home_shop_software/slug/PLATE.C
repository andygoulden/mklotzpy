/* PLATE.C   M. W. Klotz  6/03

Chain drilling a circular metal plate from flat stock
...
{
int i,j,k,n;
dbl d1,d2,d3,r3,dh,rh,h,w,theta,phi,omega,chord;

/* -------------------------- initializations ------------------------------ */
/* ------------------------------------------------------------------------- */

puts ("CHAIN DRILLING A CIRCULAR PLATE\n");

vin ("Diameter of final plate",&d1,"lf",3.,"in");
vin ("Radial allowance for finish machining",&h,"lf",0.050,"in");
vin ("Drill diameter",&dh,"lf",0.25,"in");
vin ("Approximate web thickness",&w,"lf",0.050,"in");

rh=0.5*dh; d2=d1+2.0*h; d3=d2+dh; r3=0.5*d3;
theta=2.0*ASN(rh/r3); phi=w/r3; omega=theta+phi;
n=floor(TWOPI/omega); omega=TWOPI/n; w=r3*(omega-theta);
chord=d3*sin(0.5*omega);

print;
printf ("Number of holes = %d\n",n);
printf ("Diameter of drilling circle = %.3lf in\n",d3);
printf ("Final web thickness = %.3lf in\n",w);
printf ("Angle between adjacent holes = %.3lf deg\n",DPR*omega);
printf ("Chordal distance between adjacent holes = %.3lf in\n",chord);
