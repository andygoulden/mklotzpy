/* FTAPER.C   M. W. Klotz  12/05, 3/07

Measurement of female taper with two spheres of different diameter
...
{
int i,j,k;
dbl ds,dl,hs,hl,x,rs,rl,phi,z;

/* -------------------------- initializations ------------------------------ */
/* ------------------------------------------------------------------------- */

puts ("\nMEASUREMENT OF FEMALE TAPERS WITH TWO SPHERES\n");

vin ("Diameter of small sphere",&ds,"lf",0.5,"");
vin ("Depth to top of small sphere",&hs,"lf",2.0,"");
vin ("Diameter of large sphere",&dl,"lf",0.75,"");
vin ("Depth to top of large sphere",&hl,"lf",1.0,"");
rs=0.5*ds; rl=0.5*dl; x=hs+rs-(hl+rl); z=(rl-rs)/x; phi=ASND(z);

printf ("\nTaper half angle = %.4lf deg = %.4lf in/in\n",phi,z);
printf ("Taper included angle = %.4lf deg = %.4lf in/in\n",2.*phi,2.*z);
