/* FLUTE.C   M. W. Klotz   4/09

Calculate inclination angle needed to mill tapered flutes with a ball mill
...
dbl d;		// ball mill diameter
dbl r;		// ball mill radius
dbl rs;		// flute radius at small end
dbl rf;		// flute radius at large end
dbl l;		// length of flute
dbl zs;		// tool depth at small end
dbl zf;		// tool depth at large end
dbl theta;	// workpiece inclination angle
...
agin:
vin ("Ball mill diameter",&d,"lf",0.5,"in"); r=0.5*d;
vin ("Flute radius at small end",&rs,"lf",0.1,"in");
vin ("Flute radius at large end",&rf,"lf",0.2,"in");

if ((rs > r) || (rf > r))
{printf ("\nFlute radii must be less than ball mill radius = %.4lf\n",r); goto agin;}

vin ("Length of flute",&l,"lf",3.,"in");

zs=r-sqrt(r*r-rs*rs); zf=r-sqrt(r*r-rf*rf);

printf("\nDepth of cut at small end of flute = %.4lf in\n",zs);
printf("Depth of cut at large end of flute = %.4lf in\n",zf);

theta=ATAND((zf-zs)/l);

printf("Required workpiece inclination angle = %.4lf deg =",theta); dms(theta); print;
...
if (s == 60) {s=0; m++;}
if (m == 60) {m=0; d++;}
printf ("%1c%3i deg, %2i min, %2i sec",sgn,d,m,s);
}
/* ------------------------------------------------------------------------ */
