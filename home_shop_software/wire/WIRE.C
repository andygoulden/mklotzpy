/* WIRE.C   M. W. Klotz  11/98

Electrical wire size calculations
...
#define R 1.12294049
/* diameter in mils given gage number */
#define diam(g) (324.87/pow(R,(g)))
/* gage number given diameter in mils */
#define gage(d) ((LOG10(0.32487)-LOG10(1.e-3*(d)))/LOG10(R))
dbl current,density,g;
...
puts ("ELECTRICAL WIRE COMPUTATIONS\n");
vin ("Current wire must carry",&current,"lf",12.0,"amps");
vin ("Desired current density",&density,"lf",0.0025,"amps/cmil recommended");
g=gage(sqrt(current/density));
y=modf (g,&x); if (y >= 0.5) g=x+1.; else g=x;	/* round to integer */
printf ("\nRecommended AWG gage = %.0lf\n\n",g);
d=diam(g);
printf ("Diameter = %.1lf mils\n",d);
printf ("Area = %.1lf circular mils\n",d*d);
printf ("Resistance = %.3lf ohms/1000 ft\n",10370/(d*d));
printf ("Weight = %.1lf lbs/1000 ft\n",3.02675e-3*d*d);
