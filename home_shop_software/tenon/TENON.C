/* TENON.C   M. W. Klotz   6/00

Depth-of-cut to make regular polygon tenon on end of cylindrical stock.
...
{
int i,j,k,n;
dbl ds,dc,daf,theta,doc,x;

/* -------------------------- initializations ------------------------------ */
textscreen ();
/* ------------------------------------------------------------------------- */

puts ("DEPTH OF CUT FOR REGULAR POLYGONAL TENON\n");

vin ("Diameter of stock",&ds,"lf",.5,"");
vin ("Number of sides on tenon",&n,"d",5.,"");
theta=360./n;

if (n % 2 == 0)
{
vin ("Distance across flats",&daf,"lf",.25,"");
dc=daf/COSD(0.5*theta); x=0.5*daf;
}
else
{
vin ("Diameter of circle circumscribing tenon",&dc,"lf",.25,"");
x=0.5*dc*COSD(0.5*theta);
}
doc=.5*ds-x;

print;
printf ("Stock diameter = %.4lf\n",ds);
printf ("Number of flats on tenon = %d\n",n);
printf ("Angle between adjacent tenon flats = %.2lf deg\n",theta);
printf ("Diameter of circle circumscribing tenon = %.4lf\n",dc);
printf ("Depth of cut = %.4lf\n",doc);
/* post run processing */
mexit:
exit(0);			/* normal exit */
}
