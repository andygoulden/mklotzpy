/* STICK.C   M. W. Klotz   5/01

Program to implement Peter Lott's technique (Machinist's Workshop, 6/01, pg.
18) to calculate carriage movement needed to recapture thread when cutting
metric threads on an Imperial leadscrew lathe.  This version covers all the
possible combinations of metric/imperial leadscrew and metric/imperial thread
to be cut.
...
char opl,opt;
int i,j,k;
dbl lsp;		// leadscrew pitch (tpi)
dbl tp;			// thread pitch (tpi or mm)
dbl g,stick,a,b,c,d,f;
dbl low,high;

/* -------------------------- initializations ------------------------------ */
/* ------------------------------------------------------------------------- */

puts ("THREAD CHASING OFFSETS\n");

opl=getkey ("Type of leadscrew thread (M)etric or [I]mperial?");
if (opl == 'm')
{vin ("Leadscrew pitch",&lsp,"lf",4.,"mm"); a=25.4/lsp;}
else
{opl='i'; vin ("Leadscrew pitch",&lsp,"lf",10.,"tpi"); a=lsp;}

opt=getkey ("Type of thread [M]etric or (I)mperial?");
if (opt == 'i')
{vin ("Pitch of thread being cut",&tp,"lf",15.,"tpi"); b=tp;}
else
{opt='m'; vin ("Pitch of thread being cut",&tp,"lf",4.,"mm"); b=25.4/tp;}

// gonna move one leadscrew thread at a time - thus a*stick always an integer
c=1./a; stick=c;	
low=0.000001; high=1.-low;
agin:
{
if (kbhit()) goto mexit;	// bailout in case of runaway
f=modf(b*stick,&d);		// fractional part of b*stick
if (f < low || f > high) goto done;  // b*stick must be an integer
stick+=c; goto agin;
}

done:
printf ("\nMove carriage %.3lf in = %.3lf mm to right to recapture thread\n",stick,25.4*stick);
