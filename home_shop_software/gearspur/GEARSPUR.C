/* gearspur.C   M. W. Klotz   1,2,4,11/06, 1/07, 1/10

Calculate various spur gear parameters
...
dbl OD;			/* outside diameter */
dbl A;			/* addendum */
dbl D;			/* dedendum */
dbl W;			/* whole depth */
dbl CP;			/* circular pitch */
dbl T;			/* tooth thickness */
dbl PD;			/* pitch diameter */
dbl module;		// module
...
ut=getkey("[I]mperial or (M)etric units?");
if (ut != 'm') ut='i';
if (ut == 'm') strcpy (u,"mm"); else strcpy (u,"in");

agin: k=0; OD=N=PD=P=0;

puts ("\nEnter whatever data you know.  Enter zero (0) for unknowns.");
puts ("You must enter two data items to obtain an answer.\n");

vin ("OD of gear",&OD,"lf",2.35,u); if (OD) k++;
if (u[0] == 'm') OD/=25.4;
vin ("Number of teeth",&N,"lf",45.,""); if (N) k++;
if (k == 2) goto compute;
vin ("Diametral Pitch",&P,"lf",20.,""); if (P) k++;
if (k == 2) goto compute;
vin ("Module",&module,"lf",0.7874,""); if (module) {k++; P=25.4/module;}
if (k == 2) goto compute;
vin ("Pitch Diameter",&PD,"lf",2.25,u); if (PD) k++;
if (u[0] == 'm') PD/=25.4;
if (k == 2) goto compute;
else
{
beep; puts ("\nINSUFFICENT DATA FOR SOLUTION. TRY AGAIN."); goto agin;
}

compute:
if (N && OD) {P=(N+2.)/OD; PD=N/P; goto finish;}
if (P && N) {OD=(N+2.)/P; PD=N/P; goto finish;}
if (P && OD) {N=P*OD-2.; PD=N/P; goto finish;}
if (PD && N) {P=N/PD; OD=(N+2.)/P; goto finish;}
if (PD && OD) {N=2.*PD/(OD-PD); P=N/PD; goto finish;}
if (PD && P) {N=P*PD; OD=(N+2.)/P; goto finish;}
...
printf ("Outside Diameter = %.4lf in = %.4lf mm\n",OD,OD*25.4);
printf ("Pitch Diameter = %.4lf in = %.4lf mm\n",PD,PD*25.4);
printf ("Addendum = %.4lf in = %.4lf mm\n",A,A*25.4);
printf ("Dedendum = %.4lf in = %.4lf mm\n",D,D*25.4);
printf ("Whole Depth = %.4lf in = %.4lf mm\n",W,W*25.4);
printf ("Circular Pitch = %.4lf in = %.4lf mm\n",CP,CP*25.4);
printf ("Tooth Thickness = %.4lf in = %.4lf mm\n",T,T*25.4);
...
/* return number of Brown and Sharpe cutter needed to cut gear with n teeth */
