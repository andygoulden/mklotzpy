/* BUCKET.C   M. W. Klotz  11/99

Calculations for frustrum of a cone
...
{
int i,j,k,n;
dbl db,ds,rb,rs,sh,h,theta,vol,vi,t,hi,shi,ri,eps,ve,dh;

/* -------------------------- initializations ------------------------------ */
textscreen ();
/* ------------------------------------------------------------------------- */

puts ("BUCKET CALCULATIONS\n");

vin ("Diameter of bucket big end",&db,"lf",4.,"");
vin ("Diameter of bucket small end",&ds,"lf",3.,"");
vin ("Slant height of bucket",&sh,"lf",6.,"");
vin ("Divide volume into how many parts",&n,"d",4.,"");
printf ("\n");
...
t=ri*ri+rs*ri+rs*rs;
ve=PI*hi*t/3.;
if (++k > 500) {puts ("iteration error"); quit;}
if (ABS(vi-ve) >= eps)
{
hi=hi+dh*SGN(vi-ve);
goto agin;
...
cont:
shi=hi/COSD(theta);
printf ("Slant height for %d/%d of total volume = %.3lf\n",i,n,shi);
}
