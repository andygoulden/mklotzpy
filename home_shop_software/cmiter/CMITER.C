/* CMITER.C   M. W. Klotz   3/02

Compute compound tablesaw angles for cutting pieces for mitred polygonal
forms.  Written for Jeff Jarnberg (jjarnberg@cox.net)
...
{
int i,j,k,n;
dbl s,mg,btm,btb,cmg,ts;


/* -------------------------- initializations ------------------------------ */
/* ------------------------------------------------------------------------- */

puts ("TABLE SAW ANGLES FOR POLYGONAL FORMS\n");

vin ("Number of sides",&n,"d",4.,"");
vin ("Slope",&s,"lf",30.,"deg");
print;

printf ("Number of sides = %d\n",n);
printf ("Slope = %.4lf deg\n",s);
if (s == 90.0)
{
mg=90.;
btm=180./n; btb=0;
...
mg=ATAND(1./(COSD(s)*TAND(180./n)));
cmg=COSD(mg); ts=TAND(s);
btm=ATAND(cmg*ts);
btb=ATAND(cmg/ts);

done:
printf ("Miter gauge angle = %.4lf deg\n",mg);
printf ("Blade tilt (mitered) = %.4lf deg\n",btm);
printf ("Blade tilt (butted) = %.4lf deg\n",btb);
