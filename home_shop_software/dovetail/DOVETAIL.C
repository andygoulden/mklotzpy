/* DOVETAIL.C   M. W. Klotz   3/01, 1/06

Measuring dovetails with cylinders
...
char mf;
int i,j,k;
dbl angle,pin,x,t,b,h,c,ta,q;

/* -------------------------- initializations ------------------------------ */
textscreen ();
/* ------------------------------------------------------------------------- */

puts ("DOVETAIL MEASUREMENTS\n");

agin:
mf=getkey ("(M)ale or (F)emale dovetail ? ");
if (mf != 'm' && mf != 'f') {beep; puts ("error, try again"); goto agin;}
vin ("Dovetail angle",&angle,"lf",60.,"deg");
vin ("Pin diameter",&pin,"lf",0.375,"");
if (mf == 'm')
{
vin ("Height of dovetail",&h,"lf",0.5,"");
vin ("Measurement across pins",&x,"lf",2.5,"");
}
else
{
vin ("Depth of dovetail",&h,"lf",0.5,"");
vin ("Measurement between pins",&x,"lf",1.0283,"");
}

c=h/TAND(angle); ta=TAND(0.5*angle); q=1.+1./ta;
...
print;
printf ("Dovetail angle = %.4lf deg\n",angle);
printf ("Pin diameter = %.4lf\n",pin);
printf ("Measurement across pins = %.4lf\n",x);
printf ("Height of dovetail = %.4lf\n",h);
printf ("Measurement across top of dovetail = %.4lf\n",t);
printf ("Measurement across bottom of dovetail = %.4lf\n",b);
