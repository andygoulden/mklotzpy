/* DIVHEAD.C   M. W. Klotz   3/99

Dividing head computations
...
puts ("DIVIDING HEAD CALCULATIONS\n");

if (rdata ()) goto mexit;		/* read the data file */
/* printf ("Number of data items read = %d\n",nd); */

vin ("Number of workpiece divisions",&divisions,"d",14.,"");
turns=(dbl)ratio/(dbl)divisions;
fint=0; ld=divisions;
while (fint*ld <= ratio) fint++;
fint--;
ln=ratio-fint*ld;
...
printf ("a plate with an integer multiple of %d holes is required",ld);
