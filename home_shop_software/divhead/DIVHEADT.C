/* DIVHEADT.C   M. W. Klotz   3/99, 3/05

Dividing head computations.  This version creates a table of turns/holes for
all divisions up to a user input maximum.
...
int ratio;		/* dh worm gear ration (e.g., 40:1) */
int divisions;		/* number of angular divisions of workpiece */
int ndivisions;		// maximum number of divisions
dbl turns;		/* number of index crank turns */
dbl g;
int fint,ln,ld;

FILE *fpt;			/* utility file pointer */
char dfile[]="DIVHEAD.DAT";	/* input data file */
char ofile[]="DIVHEADT.OUT";	// output file
...
puts ("DIVIDING HEAD TURNS/HOLES TABLE GENERATOR\n");

if (rdata ()) goto mexit;		/* read the data file */
/* printf ("Number of data items read = %d\n",nd); */

if ((fpt=fopen(ofile,"wt")) == NULL)
	{printf ("FAILED TO OPEN OUTPUT FILE: %s\n",ofile); quit;}

vin ("Maximum number of divisions for output table",&ndivisions,"d",50.,"");

fprintf (fpt,"Turns & holes/plate for dividing head with ");
fprintf (fpt,"worm gear ratio = %d:1\n",ratio);
fprintf (fpt,"\nAvailable hole plates =\n");

for (i=0 ; i<nh ; i++) fprintf (fpt,"%d, ",holes[i]);
fprintf (fpt,"\n\n");

for (divisions = 2; divisions<=ndivisions; divisions++)
{
...
fprintf (fpt,"%d =>",divisions);


/* report general solution if it exists */
if (ln == 0)
{
fprintf (fpt," %d & 0 ",fint+ln);
goto nextd;
}
...
if (!(holes[i] % ld))
	...
	if (j) fprintf (fpt," or ");
	   ...
	   {
	   fprintf (fpt," %d & ",fint); j++;
	   }
	k=holes[i]*ln/ld;
	fprintf (fpt,"%d/%d",k,holes[i]);
	}
}

if (!j)		/* solution not found */
{
fprintf (fpt," a plate with an integer multiple of %d holes is required",ld);
