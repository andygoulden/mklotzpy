/* CONROD.C   M. W. Klotz   5/01

Calculate connecting rod clearance relative to cylinder/crosshead bottom at
worst case lateral position of con rod.
Written at the suggestion of Tom Roach (troach@sn.apc.org) in South Africa who
needed it for designing two cycle engines.  However, the calculation is useful
for any engine designer.
...
{
int i,j,k;
dbl conl,radius,w,xa,phi,sp,cp,tp,d;
dbl d12,d13,d14,d23,d25,d34,d35,d45;

/* -------------------------- initializations ------------------------------ */
/* ------------------------------------------------------------------------- */

puts ("CONNECTING ROD CLEARANCE\n");

puts ("Units of measurement don't matter but must be consistent");
vin ("Connecting rod length (center-to-center)",&conl,"lf",2.4,"");
puts ("Radius measured from crank center to connecting rod driver center");
vin ("Crank radius",&radius,"lf",0.6,"");
vin ("Height of cylinder bottom above crank center",&w,"lf",1.5,"");
vin ("Cylinder diameter",&d,"lf",1.0,"");

xa=sqrt(radius*radius+conl*conl);
cp=conl/xa;
sp=radius/xa;
tp=sp/cp;
phi=ACSD(cp);
...
printf ("Phi = %.4lf deg\n",phi);
printf ("Distance, gudgeon pin to crank center = %.4lf\n",xa);
printf ("Clearance, con rod centerline to cylinder bottom = %.4lf\n",d23);
printf ("d34 = %.4lf\n",d34);
printf ("d45 = %.4lf\n",d45);
printf ("d35 = %.4lf\n",d35);
printf ("d23 = %.4lf\n",d23);
