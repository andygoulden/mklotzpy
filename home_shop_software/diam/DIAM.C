/* DIAM.C   M. W. Klotz  4/99, 2/05

Diameter as a function of material and cutting speed
...
char dfile[]="DIAM.DAT";	/* data file */
char ofile[]="DIAM.OUT";	/* output file */
#define ND 400		/* maximum number of data entries */
int nd;			/* actual number of data entries */
char name[ND][40];	/* material names */
int lo[ND],hi[ND];	/* low, high values of sfpm */
int nm;			/* material number */
dbl d;			/* diameter */
dbl low,high;		/* recommended rpm */
int ns;			/* number of machine speeds */
dbl speed[ND];		/* available machine speeds (rpm) */
dbl dlow,dhigh;		/* diameters corresponding to speeds and material */
dbl c=12/PI;
...
textscreen ();
/* ------------------------------------------------------------------------- */
puts ("MACHINING DIAMETER UTILITY\n");

rdata ();
printf ("number of data entries read = %d\n\n",nd);
for (i=0 ; i<nd ; i++)
{printf ("%4d  %s\n",i+1,name[i]); if ((i+1) % 15 == 0) pausem;}

if ((fp=fopen(ofile,"wt")) == NULL)
	{printf ("FAILED TO OPEN OUTPUT FILE: %s\n",ofile); quit;}

agin: vin ("Material Number",&nm,"%d",(dbl)1,""); nm--;
if (nm < 0 || nm > nd-1) {beep; puts ("INVALID NUMBER"); goto agin;}
...
for (i=0 ; i<ns ; i++)
{
dlow=c*lo[nm]/speed[i]; dhigh=c*hi[nm]/speed[i];
fprintf (fp,"%6.0lf  %6.3lf  %6.3lf\n",speed[i],dlow,dhigh);
}
...
while ((fgets(temp,sizeof(temp),fp)) != NULL)
	...
	if ((strstr(temp,"SPEEDS")) != NULL) {f=1; continue;}
	if (temp[0] != '\0' && temp[0] != ';')
		{
		if (f)
		{
		speed[ns]=atof(strtok(temp,"\t;"));
		if (++ns == ND)
			{
			printf
		...
		lo[nd]=atof(strtok(NULL,"\t;"));
		hi[nd]=atof(strtok(NULL,"\t;"));
		if (++nd == ND)
			{
			printf
			("More than maximum (%d) data items in data file\n",ND);
