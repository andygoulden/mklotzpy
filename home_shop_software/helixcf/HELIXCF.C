/* helixcf.C   M. W. Klotz   7/10

Chuck Fellows' calculations for helical gears
...
dbl HA;			// helix angle
dbl HL;			// helix lead
dbl MD;			// mandrel diameter
dbl TT;			// template thickness
dbl TA;			// template angle
...
vin ("\nNumber of teeth",&N,"lf",6.,"");
vin ("Diametral Pitch",&P,"lf",40.,"");
vin ("Helix angle",&HA,"lf",80.,"deg");
vin ("Mandrel hub diameter",&MD,"lf",1.,"in");
vin ("Template thickness",&TT,"lf",0.125,"in");

PD = N/(P*COSD(HA));
HL = PI*PD/TAND(HA);
OD=PD+2./P;
if (P <= 20) W=2.2/P+.002; else W=2.157/P;
TA=ATAND(HL/(PI*MD+TT));

printf ("\nDiametral Pitch = %.4lf\n",P);
printf ("Number of teeth = %.0lf\n",N);
printf ("Helix Angle = %.4lf deg\n",HA);
printf ("Gear Blank Diameter = %.4lf in\n",OD);
printf ("Whole Depth = %.4lf in\n",W);
printf ("Pitch Diameter = %.4lf in\n",PD);
printf ("Helix Lead = %.4lf in\n",HL);
printf ("Template angle = %.4lf deg\n",TA);
