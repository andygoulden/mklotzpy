...
char ofile[]="LOAN.OUT";	/* output file */
...
{
int i,n;
dbl p0,per,r,s,m,ptot,p1,p2,q;

/* -------------------------- initializations ------------------------------ */
textscreen ();
/* ------------------------------------------------------------------------- */

puts ("LOAN CALCULATIONS\n");

vin ("Loan amount",&p0,"lf",10000.,"$");
vin ("Number of payment periods",&n,"d",10.,"");
vin ("Interest rate",&per,"lf",6.,"%/period");
r=0.01*per; s=1.+r; q=pow(s,(dbl)n); m=p0*r*q/(q-1.);
ptot=m/r-(m/r-p0)*q+n*m;

if ((fp=fopen(ofile,"wt")) == NULL)
	{printf ("FAILED TO OPEN OUTPUT FILE: %s\n",ofile); quit;}

fprintf (fp,"LOAN CALCULATIONS\n\n");
fprintf (fp,"Principal = $%.2lf\n",p0);
fprintf (fp,"Periods = %d\n",n);
fprintf (fp,"Interest = %.2lf %%\n\n",per);
fprintf (fp,"Payment each period = $%s\n",c(m));
fprintf (fp,"Total loan cost = $%s\n\n",c(ptot));
fprintf (fp,\
"PERIOD       PRINCIPAL       INTEREST         PTOTAL         ITOTAL\n\n");
for (i=1 ; i<=n ; i++)
{
p1=m/r-(m/r-p0)*pow(s,(dbl)i);
p2=m/r-(m/r-p0)*pow(s,(dbl)(i-1));
fprintf (fp,"%-7d%15s",i,c(m-p2*r));
fprintf (fp,"%15s",c(p2*r));
fprintf (fp,"%15s",c(p0-p1));
fprintf (fp,"%15s\n",c(i*m-p0+p1));
}

fclose (fp);

sprintf (gp,"MORE < %s",ofile); system (gp);
...
l=strrchr(gp,'.')-3;
for (b=l ; b>&gp[0] ; b-=3)
  {if isdigit ((int)(*(b-1))) {strcpy (fp,b); *b=','; strcpy (b+1,fp);}}
return gp;
}
/* ------------------------------------------------------------------------ */
