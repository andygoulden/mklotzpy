/* BUOY.C   M. W. Klotz  8/05

Determine how deeply buoys of various shapes will sink in various liquids
...
dbl rhol;		// liquid density (lb/in^3)
dbl weight;		// weight of buoy (lb)
dbl dc;			// diameter of sphere/cylinder (in)
dbl lc;			// length of cylinder (in)
dbl volume;		// volume of buoy (in^3)
dbl force;		// buoy force (lb)
dbl h;			// immersion depth (in)
dbl wc,hc;
...
puts ("Densities (lb/in^3):");
puts ("Sea Water = 0.037");
puts ("Water = 0.0361");
puts ("Mercury = 0.4894");
print;

vin ("Density of liquid",&rhol,"lf",0.0361,"lb/in^3");
vin ("Weight of buoy and applied load",&weight,"lf",3.5,"lb");

print;
puts ("Buoy shape:");
puts ("a - Sphere");
puts ("b - Horizontal Cylinder");
puts ("c - Vertical Cylinder");
puts ("d - Rectangular Parallelepiped");


agin: op=getkey("(a-d) ? "); print;
switch (op)
{
case 'a':		// sphere
	vin ("Diameter of sphere",&dc,"lf",10.,"in");
	volume=PI*dc*dc*dc/6.;
	force=volume*rhol;
	if (force<weight) {error(force); goto mexit;}
	h=binsearch (0.,dc,weight,0.01);
	break;
case 'b':		// horizontal cylinder
	vin ("Diameter of cylinder",&dc,"lf",5.,"in");
	vin ("Length of cylinder",&lc,"lf",10.,"in");
	volume=0.25*PI*dc*dc*lc;
	force=volume*rhol;
	if (force<weight) {error(force); goto mexit;}
	h=binsearch (0.,dc,weight,0.1);
	break;
case 'c':		// vertical cylinder
	vin ("Diameter of cylinder",&dc,"lf",5.,"in");
	vin ("Length of cylinder",&lc,"lf",10.,"in");
	volume=0.25*PI*dc*dc*lc;
	force=volume*rhol;
	if (force<weight) {error(force); goto mexit;}
	h=weight/(0.25*PI*dc*dc*rhol);
	break;
case 'd':		// rectangular parallelepiped
	vin ("Length of buoy",&lc,"lf",5.,"in");
	vin ("Width of buoy",&wc,"lf",8.,"in");
	vin ("Height of buoy",&hc,"lf",6.,"in");
	volume=lc*wc*hc;
	force=volume*rhol;
	if (force<weight) {error(force); goto mexit;}
	h=weight/(lc*wc*rhol);
...
{
puts ("Buoy will sink!");
printf ("A buoy of this size can support <= %.4lf lb\n",f);
}
/* binsearch..------------------------------------------------------------- */
...
{
	case 'a':	// sphere
		y=rhol*PI*x*x*(1.5*dc-x)/3;
		break;
	case 'b':	// horizontal cylindrical tank
		y=rhol*fcyl(x)*lc;
		break;
	case 'c':	// vertical cylindrical tank
		y=rhol*0.25*PI*dc*dc*x;
		break;
}
if (ABS(y-val) <= tol) return x;
if (val < y) r=x; else l=x;
if (++i>500) return -1;		// prevent runaway
goto agin;
}
/* fcyl..----------------------------------------------------------------- */
