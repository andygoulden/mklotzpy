/* LINKS.C   M. W. Klotz   10/00

Calculations for tapered, radiused-end links
...
{
int i,j,k;
dbl r1,r2,d1,d2,d,h,theta1,theta2,theta3;

/* -------------------------- initializations ------------------------------ */
textscreen ();
/* ------------------------------------------------------------------------- */

puts ("CALCULATIONS FOR TAPERED, RADIUSED-END LINKS\n");
vin ("Small end radius",&r1,"lf",1/16.,"in");
vin ("Small end hole diameter",&d1,"lf",1/16.,"in");
vin ("Big end radius",&r2,"lf",3./32.,"in");
vin ("Big end hole diameter",&d2,"lf",3./16.,"in");
vin ("Distance between hole centers",&d,"lf",1.,"in");
theta1=ASND((r2-r1)/d); theta2=180.-2*theta1; theta3=180.+2*theta1;
h=r2-r1+0.5*(d2-d1);
print;
printf ("Angle of tapered side = %.2lf deg\n",theta1);
printf ("Included angle of small end = %.2lf deg\n",theta2);
printf ("Included angle of big end = %.2lf\n",theta3);
printf ("Small end shim height = %.4lf in\n",h);
