/* COLLET.C   M. W. Klotz   5/04

Calculations for making cylindrical collets to accept polygonal stock.
Reference:  Machinist's Workshop, June/July 2004, Vol. 17, Number 3.
...
int n;			// polygon number of sides
dbl af;			// across flat dimension of stock
dbl sw;			// slot width
dbl theta;		// half central angle of polygon
dbl d;			// collet bore diameter
...
puts ("\nCYLINDRICAL COLLETS FOR POLYGONAL STOCK\n");

vin ("Number of stock polygon sides",&n,"d",6.,"");
vin ("Stock across flats dimension",&af,"lf",3./16.,"in");
vin ("Collet slot width",&sw,"lf",0.045,"in");

theta=180./n; ct=COSD(theta); st=SIND(theta);

d=2.*sqrt(0.25*sw*sw+SQR(0.5*af/ct-0.5*sw*st/ct));

printf ("\nRequired collet bore diameter = %.4lf in\n",d);
