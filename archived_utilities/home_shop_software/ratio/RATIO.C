...
char dfile[]="DATA.DAT";	/* input data file */
char ofile[]="RATIO.OUT";	/* output file */
#define ND 100			/* maximum number of data items */
int nd;				/* actual number of data items */
int op,md,m;
int mm[5]={5,10,20,40,80};
dbl scale;
...
puts ("AMERICAN NATIONAL STANDARD FOR PREFERRED NUMBERS\n");

md=0;
puts ("1 -  5 NUMBERS ABOUT 58% APART");
puts ("2 - 10 NUMBERS ABOUT 26% APART");
puts ("3 - 20 NUMBERS ABOUT 12% APART");
puts ("4 - 40 NUMBERS ABOUT  6% APART");
puts ("5 - 80 NUMBERS ABOUT  3% APART");
puts ("6 - DESIGN YOUR OWN SET OF NUMBERS");

vin ("Option",&op,"d",(dbl)2,"");
if (op == 6) vin ("How many numbers in series",&md,"d",(dbl)15,"");

puts
  ("\nThe first number in each series is always 1.  You can scale the series");
puts ("by applying a scale factor (typically a power of ten) to each number");
puts ("in the series.\n");
vin ("Scale factor",&scale,"lf",10.,"");

if (md) m=md; else m=mm[op-1];

wdata ();

printf ("Your data is on: %s\n",ofile);
...
fprintf (fp,"AMERICAN NATIONAL STANDARD FOR PREFERRED NUMBERS\n\n");
fprintf (fp,"RATIO OF CONSECUTIVE NUMBERS = %.2lf %%\n\n",\
  100*(pow(10.,1./(dbl)m)-1.));
fprintf (fp,"N     NUMBER      SCALED       NEAREST INTEGER\n");
for (i=0 ; i<m ; i++)
{
r=pow(10.,(dbl)i/(dbl)m); s=scale*r;
if (modf(s,&x) > 0.5) l=(ulong)(x+1.); else l=(ulong)x;
fprintf (fp,"%-3d   %-10.4lf  %-10.4lf   %-10lu\n",i,r,s,l);
}
