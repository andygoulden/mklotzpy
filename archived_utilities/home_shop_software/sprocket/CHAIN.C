/* CHAIN.C   M. W. Klotz   2/03

Find sprocket center-to-center distance for a given chain length
...
dbl p;		//chain pitch (in)
dbl l;		//chain length (pitches)
int nl;		//number of teeth in large sprocket
int ns;		//number of teeth in small sprocket
dbl c;		//center-to-center distance for mounting sprockets
...
puts ("\nSPROCKET CENTER DISTANCE GIVEN CHAIN LENGTH\n");

vin ("Chain pitch",&p,"lf",1.,"in");
vin ("Chain length",&l,"lf",48.,"pitches");
vin ("Number of teeth in large sprocket",&nl,"d",18.,"");
vin ("Number of teeth in small sprocket",&ns,"d",9.,"");

a=2.*l-nl-ns; b=nl-ns;
c=(p/8.)*(2.*l-nl-ns+sqrt(a*a-0.810*b*b));

printf ("\nSprocket center-to-center distance = %.4lf in\n",c);
