/* HORSE.C   M. W. Klotz   4/01

Compute relations between torque, horsepower and rotational speed.
...
{
int k;
dbl torque=0.,hp=0.,rpm=0.;

/* -------------------------- initializations ------------------------------ */
textscreen ();
/* ------------------------------------------------------------------------- */

puts ("TORQUE-SPEED-HORSEPOWER CALCULATIONS\n");

puts ("Input whatever data you know; press return if not known.");
puts ("You must enter two data items to obtain a solution.\n");
k=0;
puts ("ft-lb = in-lb/12");
vin ("Torque",&torque,"lf",0.,"ft-lb"); if (torque) k++;
vin ("Rotational speed",&rpm,"lf",0.,"rpm"); if (rpm) k++;
if (k == 2) goto doit;
vin ("Horsepower",&hp,"lf",0.,"hp"); if (hp) k++;
if (k == 2) goto doit;
...
if (hp && rpm) {torque=hp*5252./rpm; goto disp;}

if (hp && torque) {rpm=hp*5252./torque; goto disp;}

if (torque && rpm) {hp=torque*rpm/5252.; goto disp;}

disp:
puts ("");
printf ("Torque     = %.4lf ft-lb = %.4lf in-lb\n",torque,torque*12.);
printf ("Speed      = %.4lf rpm\n",rpm);
printf ("Horsepower = %.4lf hp\n",hp);
