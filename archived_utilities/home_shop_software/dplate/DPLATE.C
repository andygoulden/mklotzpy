/* DPLATE.C   M. W. Klotz   1/02

Calculations to construct a dividing plate from cylindrical disks arranged
...
{
int ndiv;
dbl dc,rc,rd,theta,s;

/* -------------------------- initializations ------------------------------ */
/* ------------------------------------------------------------------------- */

puts ("DIVISION PLATE CALCULATION\n");

vin ("Number of divisions",&ndiv,"d",14.,"");
vin ("Diameter of mounting circle",&dc,"lf",112.,"");
rc=0.5*dc;
theta=360./ndiv;
s=SIND(0.5*theta);
rd=rc*s/(1.-s);
printf ("Disk diameter = %lf\n",2.*rd);
