/* BOLTCIRC.C   M. W. Klotz   11/98, 1/00

Boltcircle computations
...
{
int i,j,k,nh;
dbl rc,dh,sp,space,ang,a0,x,y,x0,y0;

/* -------------------------- initializations ------------------------------ */
textscreen ();
/* ------------------------------------------------------------------------- */

puts ("BOLTCIRCLE COMPUTATIONS\n");
vin ("Number of holes",&nh,"d",5.,"");
vin ("Radius of bolt circle",&rc,"lf",1.0,"");
vin ("Diameter of bolt holes",&dh,"lf",0.5,"");
vin ("Angular offset of first hole",&a0,"lf",0.0,"deg");
vin ("X offset of bolt circle center",&x0,"lf",0.0,"");
vin ("Y offset of bolt circle center",&y0,"lf",0.0,"");
sp=360./nh; space=2.*rc*SIND(0.5*sp)-dh;
if (space < 0.) {beep; puts ("\nWARNING: HOLES WILL OVERLAP !"); }

if ((fp=fopen(ofile,"wt")) == NULL)
	{printf ("FAILED TO OPEN OUTPUT FILE: %s\n",ofile); quit;}

fprintf (fp,"Boltcircle specification:\n");
fprintf (fp,"Radius of bolt circle = %.4lf\n",rc);
fprintf (fp,"Bolt hole diameter = %.4lf\n",dh);
fprintf (fp,"Spacing between hole edges = %.4lf\n",space);
fprintf (fp,"Angular offset of first hole = %.4lf deg\n",a0);
fprintf (fp,"X offset of bolt circle center = %.4lf\n",x0);
fprintf (fp,"Y offset of bolt circle center = %.4lf\n\n",y0);
fprintf (fp,"HOLE       ANGLE     X-COORD     Y-COORD\n\n");
for (i=0 ; i<nh ; i++)
{
ang=a0+i*sp;
fprintf (fp,"%4d  %10.4lf  %10.4lf  %10.4lf\n",\
i+1,ang,rc*COSD(ang)+x0,rc*SIND(ang)+y0)+y0;
}
printf ("\nYour data is on: %s\n",ofile);
