...
{
int i,j,k;
dbl speed,ipr,ipm,load,edges;

/* -------------------------- initializations ------------------------------ */
textscreen ();
/* ------------------------------------------------------------------------- */

puts ("FEED RATE CALCULATIONS\n");

puts ("Input whatever data you know; press return if not known.");
puts ("You must enter three data items to obtain a solution.");
puts ("If finding speed from edges, chip load and feed,");
puts ("feed must be entered in in/min.");
print;
k=0;
vin ("Speed",&speed,"lf",0.,"rpm"); if (speed) k++;
vin ("Number of cutting edges",&edges,"lf",0.,"edges/revolution"); if (edges) k++;
vin ("Chip load",&load,"lf",0.,"in/cutting edge"); if (load) k++;
if (k == 3) goto doit;
vin ("Feed",&ipm,"lf",0.,"in/min"); if (ipm) k++;
if (k == 3) goto doit;
if (edges && load) goto whoops;
vin ("Feed",&ipr,"lf",0.,"in/revolution"); if (ipr) k++;
if (k == 3) goto doit;
...
if (speed && edges && (ipr || ipm))
{
if (ipm) ipr=ipm/speed;
if (ipr) ipm=ipr*speed;
load=ipm/(speed*edges);
goto disp;
}

if (speed && load && (ipr || ipm))
{
if (ipm) ipr=ipm/speed;
if (ipr) ipm=ipr*speed;
...
disp:
print;
printf ("Cutting edges = %.0lf\n",edges);
printf ("Speed = %.0lf rpm\n",speed);
printf ("Chip load = %.4lf in/edge\n",load);
printf ("Feed = %.4lf in/min\n",ipm);
printf ("Feed = %.4lf in/rev\n",ipr);
