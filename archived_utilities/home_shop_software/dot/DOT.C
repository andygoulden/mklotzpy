/* DOT.C   M. W. Klotz  2/99, 12/02, 3/04, 6/05
...
{
int i,j,k;
dbl ta,tpi,pitch,dot,dot29,h,rang,cang,fracp,intp;
dbl dotscsr,dotfcfr,dotscfr,dotfcsr,undercut,z;

/* -------------------------- initializations ------------------------------ */
textscreen ();
/* ------------------------------------------------------------------------- */

puts ("DEPTH OF THREAD CALCULATIONS\n");

vin ("Threads angle",&ta,"lf",60.,"deg");
vin ("Threads per inch",&tpi,"lf",20.,"");
vin ("Compound rest angle",&rang,"lf",29.,"deg");
cang=COSD(rang);
pitch=1./tpi;
h=dotscsr=.5*pitch/TAND(0.5*ta);	/* dot sharp crest to sharp root */
dotfcfr=.625*h;			/* dot flat crest to flat root */
dotscfr=.75*h;			/* dot sharp crest to flat root */
dotfcsr=.875*h;			/* dot flat crest to sharp root */
/* undercut below nominal diameter for class 2A and 2B (female) threads */
undercut=.25*h;			/* 2 * h/8 */

print;
printf ("thread angle = %.2lf deg\n",ta);
printf ("threads per inch = %.1lf",tpi);
printf ("   (pitch = %.5lf in/thread)\n",pitch);
printf ("{compound feed at compound angle = %.1lf deg}\n\n",rang);
printf ("(A) dot sharp crest - sharp root = %.5lf in {%.5lf in}\n",\
dotscsr,dotscsr/cang);

if (ta != 60.)
{
puts ("\n***********************");
puts ("NB... For thread forms other than American National, the following");
puts ("information may not be valid unless the root and crest flats are defined");
puts ("as in the American National form.");
puts ("***********************\n");
}


printf ("(B) dot flat  crest - flat  root = %.5lf in {%.5lf in}\n",\
dotfcfr,dotfcfr/cang);
printf ("(C) dot sharp crest - flat  root = %.5lf in {%.5lf in}\n",\
dotscfr,dotscfr/cang);
printf ("(D) dot flat  crest - sharp root = %.5lf in {%.5lf in}\n",\
dotfcsr,dotfcsr/cang);
printf ("(E) double dot sharp crest - sharp root = %.5lf in\n",2.*dotscsr);

z=(3./16.)*pitch/TAND(30.);  // distance of pitch line below p/8 flat
printf ("\nFor American National (60 deg) thread form, subtract %.4lf in from\n",2.*z);
puts ("major diameter (assumes p/8 flat on crest) to obtain pitch diameter\n");

fracp=modf(tpi,&intp);
if (fracp) puts ("use any odd-numbered line on threading dial");
else
{
if ((int)intp % 2) puts ("use any numbered line on threading dial");
else puts ("use any line on threading dial");
}
