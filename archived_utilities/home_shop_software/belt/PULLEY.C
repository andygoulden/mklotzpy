/* PULLEY.C   M. W. Klotz   3/00

Calculate pulley size given belt length
...
printf ("TWO PULLEY BELT LENGTH CALCULATION\n");

print;
vin ("Diameter of known pulley",&d1,"lf",1.4,""); r1=0.5*d1;
vin ("Separation between pulley centers",&sep,"lf",2.5,"");
vin ("Belt length",&dlen,"lf",8.21,"");
vin ("Calculation accuracy",&acc,"lf",0.001,"");

ds=0.1*d1; df=10*d1; diff=-1;

again:
for (d2=ds ; d2<=df ; d2+=0.1*(df-ds))
{
if (kbhit()) goto mexit;
difflast=diff;
r2=0.5*d2; eps=r2-r1; phi=ASN(eps/sep); theta=2*phi;
...
if (difflast < 0 && diff > 0) {dh=d2; break;}
dl=d2;
}
ds=dl; df=dh; goto again;
