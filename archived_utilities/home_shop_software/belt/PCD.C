/* PCD.C   M. W. Klotz   11/05

Calculate pulley center distance given belt length and pulley sizes
...
{
int i,j,k;
dbl sep,d1,d2,r1,r2,eps,phi,theta,beta1,beta2,wrap1,wrap2,blen,span;
dbl dlen,acc,diff,difflast,ds,df,dl,dh;
dbl ss,sf,sl,sh;
...
printf ("TWO PULLEY CENTER DISTANCE CALCULATION\n");

print;
vin ("Diameter of driver pulley",&d1,"lf",1.4,""); r1=0.5*d1;
vin ("Diameter of driven pulley",&d2,"lf",0.603,""); r2=0.5*d2;
vin ("Belt length",&dlen,"lf",8.21,"");
vin ("Calculation accuracy",&acc,"lf",0.0001,"");

ss=0.1*dlen; sf=10*dlen; diff=-1;

again:
for (sep=ss ; sep<=sf ; sep+=0.1*(sf-ss))
{
if (kbhit()) goto mexit;
difflast=diff;
eps=r2-r1; phi=ASN(eps/sep); theta=2*phi;
beta1=PI-theta; beta2=PI+theta; wrap1=r1*beta1; wrap2=r2*beta2;
span=sqrt(sep*sep-eps*eps); blen=2*span+wrap1+wrap2; diff=blen-dlen;
if (ABS(diff) < acc) goto done;
if (difflast < 0 && diff > 0) {sh=sep; break;}
sl=sep;
}
ss=sl; sf=sh; goto again;

done:
puts ("\nFor known pulley:");
printf ("Diameter = %.3lf\n",d1);
printf ("Wrap angle = %.2lf deg\n",beta1*DPR);
printf ("Wrap length = %.3lf\n",wrap1);
puts ("\nFor calculated pulley:");
printf ("Diameter = %.3lf\n",d2);
printf ("Wrap angle = %.2lf deg\n",beta2*DPR);
printf ("Wrap length = %.3lf\n",wrap2);
printf ("\nBelt span between pulleys = %.3lf\n",span);
printf ("Belt length = %.3lf\n",blen);
printf ("Pulley center distance = %.3lf\n",sep);
