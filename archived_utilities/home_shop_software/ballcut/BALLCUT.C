/* BALLCUT.C   M. W. Klotz  11/98, 7/01, 2/05

Incremental cutting of spherical shapes on lathe
3/99 Added refinements suggested by Terry Koken
11/00 Added fixed dx
7/01 Correction to fixed dx
...
char ofile[]="BALLCUT.OUT";	/* output file */

int nc,hs,fdx;
dbl r,d,rs,ds,theta,dtheta,x,y,dc,f,dw,dx,dy,xinc;
...
puts ("INCREMENTAL SPHERE SHAPING ON LATHE\n");

vin ("Sphere diameter",&d,"lf",1.0,"in");
vin ("Stock diameter",&ds,"lf",d,"in");
op=getkey("Constant [A]ngle step or constant (X) step ? ");
if (op == 'x') fdx=1; else fdx=0;
if (!fdx) vin ("Angular increment",&dtheta,"lf",5.0,"deg");
else vin ("X increment",&xinc,"lf",0.02,"in");
r=0.5*d; rs=0.5*ds;
...
fprintf (fp,"Incremental Sphere Turning Data\n");
fprintf (fp,"Sphere diameter = %.4lf in\n",d);
fprintf (fp,"Stock diameter = %.4lf in\n",ds);
if (!fdx)
fprintf (fp,"Angular increment = %.4lf deg\n\n",dtheta);
else
fprintf (fp,"X increment = %.4lf in\n\n",xinc);
fprintf (fp,"N = cut number\n");
fprintf (fp,"XF = axial (along lathe bed) position of tool\n");
fprintf (fp,"DX = increment in x from last cut\n");
fprintf (fp,"YF = depth of cut\n");
fprintf (fp,"DY = increment in y from last cut\n");
fprintf (fp,"WD = work diameter resulting from depth of cut YF\n\n");
fprintf (fp,"%3s%9s%9s%9s%9s%9s\n\n","N","XF","DX","YF","DY","WD");

if (!fdx)
{
for (theta=0.,nc=0,dx=0.,dy=0. ; theta <=90.0 ; theta+=dtheta,nc++)
{
x=r*COSD(theta); y=r*SIND(theta); dc=rs-y; f=r-x; dw=2*y;
if (dc < 0.) break;
if (nc) {dx=f-fl; dy=dc-dcl;}
fprintf (fp,"%3d%9.3lf%+9.3lf%9.3lf%+9.3lf%9.3lf\n",nc,f,dx,dc,dy,dw);
fl=f; dcl=dc;
}
}
else
{
for (x=0.,nc=0,dy=0. ; x<=r ; x+=xinc,nc++)
{
y=sqrt(r*r-(x-r)*(x-r));
dc=rs-y; dw=2*y;
if (dc < 0.) break;
if (nc) {dx=x-xl; dy=dc-dcl;}
fprintf (fp,"%3d%9.3lf%+9.3lf%9.3lf%+9.3lf%9.3lf\n",nc,x,dx,dc,dy,dw);
xl=x; dcl=dc;
}
}
