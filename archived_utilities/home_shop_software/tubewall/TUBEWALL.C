/* TUBEWALL.C   M. W. Klotz   2/02

Calculate tube wall thickness from measurements made with outside micrometer
...
{
int i,j,k;
dbl a2,a,d,R,m,t,B,C;
dbl x,r,b;

/* -------------------------- initializations ------------------------------ */
/* ------------------------------------------------------------------------- */

puts ("MEASURING TUBE WALL THICKNESS WITH OUTSIDE MICROMETER\n");

vin ("Micrometer anvil diameter",&a2,"lf",0.249,"");
a=0.5*a2;
vin ("Tube outside diameter",&d,"lf",0.879,"");
R=0.5*d;
vin ("Micrometer measurement",&m,"lf",0.0625,"");

B=-d; C=-a*a+2*R*m-m*m;

t=-0.5*(B+sqrt(B*B-4*C));

printf ("\nTube wall thickness = %.4lf\n",t);
printf ("Tube inside diameter = %.4lf\n",d-2*t);
