/* UGROOVE.C   M. W. Klotz   4/99, 2/05, 9/06

Calculations for turning a semicircular groove into cylindrical stock.
Written for Adrian Hodgson (adrian@aphtrading.wanadoo.co.uk) in the UK.
...
char ofile[]="UGROOVE.OUT";	/* output file */
dbl rw;				/* radius of workpiece */
dbl dm;				/* diameter of ball-end mill */
dbl rm;				/* radius of ball-end mill */
dbl r;				/* rw+rm */
int aorl;			// angular or linear increment flag
dbl x;				// linear location
dbl dx;				// linear step
dbl theta;			/* angle */
dbl dtheta;			/* angle step */
...
puts ("INCREMENTAL CUTTING OF A U-SHAPED GROOVE\n");

if ((fp=fopen(ofile,"wt")) == NULL)
	{printf ("FAILED TO OPEN OUTPUT FILE: %s\n",ofile); quit;}

vin ("Groove radius",&rw,"lf",1.,"in");
vin ("Lathe tool diameter",&dm,"lf",.25,"in");

op=getkey("[A]ngular or (L)inear increment ? ");
aorl=0; if (op == 'l') aorl=1;

if (aorl)
{
vin ("Linear increment",&dx,"lf",0.0625,"in");
}
else
{
vin ("Angular increment",&dtheta,"lf",5.,"deg");
}
rm=0.5*dm; r=rw-rm;
...
fprintf (fp,"Groove radius = %.4lf in\n",rw);
fprintf (fp,"Lathe tool diameter = %.4lf in\n",dm);
fprintf (fp,"\n%6s  %6s\n","X","DOC");
if (aorl)
{
x=r;
