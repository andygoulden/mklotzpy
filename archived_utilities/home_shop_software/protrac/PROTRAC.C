/* PROTRAC.C   M. W. Klotz   02/01

Calculations for a sinebar-like protractor
...
char op;
int i,j,k;
dbl r,d,theta,zeta,delta,D;

/* -------------------------- initializations ------------------------------ */
textscreen ();
/* ------------------------------------------------------------------------- */

puts ("SINEBAR-LIKE PROTRACTOR\n");
vin ("Radius on which pins are mounted",&r,"lf",3.,"in");
vin ("Pin diameter",&d,"lf",0.25,"in");
vin ("Pin separation when protractor is closed",&delta,"lf",0.22,"in");

zeta=2.0*ASND(0.5*(d+delta)/r);

printf ("\nAngle between pins when closed = %.4lf deg\n\n",zeta);

agin:
puts ("\nA  find angle given pin separation");
puts ("D  find pin separation given angle");
puts ("Q  quit");
op=getkey("(A,D,Q) ? ");
switch (op)
{
	case 'a':
	vin ("Pin separation",&D,"lf",1.0,"in");
	theta=2.0*ASND(0.5*(d+D)/r)-zeta;
	break;

	case 'd':
	vin ("Angle",&theta,"lf",15.0,"deg");
	D=2.0*r*SIND(0.5*(theta+zeta))-d;
	break;
