/* SODDY.C   M. W. Klotz   3/02, 12/04

Compute the exterior Soddy circle given the diameters of the three circles
contained therein.
...
dbl soddy (dbl d1, dbl d2, dbl d3, int k);
...
vin ("Diameter of first circle",&d1,"lf",0.245,"");
vin ("Diameter of second circle",&d2,"lf",0.249,"");
vin ("Diameter of third circle",&d3,"lf",0.253,"");

s=soddy(d1,d2,d3,1);

printf ("Diameter of outer Soddy circle = %.7lf\n",s);

s=soddy(d1,d2,d3,-1);

printf ("Diameter of inner Soddy circle = %.7lf\n",s);
...
return ABS(d1*d2*d3/(d2*d3+d1*(d2+d3)-k*2.*sqrt(d1*d2*d3*(d1+d2+d3))));
}
/* ------------------------------------------------------------------------ */
