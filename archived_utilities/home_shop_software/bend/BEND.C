/* BEND.C   M. W. Klotz  11/98

Computing bend allowances in metal
...
puts ("BEND ALLOWANCE COMPUTATION\n");
vin ("Thickness of material",&t,"lf",0.125,"in");
vin ("Radius of bend",&r,"lf",3.0,"in");
vin ("Angle of bend",&ang,"lf",180.0,"deg");
ang*=RPD; x=.4*t; if (r < 2.*t) x=.33333*t;  if (r > 4.*t) x=.5*t;
ba=ang*(r+x);
printf ("\nLength of bend exterior = %.4lf in\n",ang*(r+t));
printf ("Length of bend interior = %.4lf in\n",ang*r);
printf ("\nLength of material required to form bend = %.4lf in",ba);
