/* PROFILE.C   M. W. Klotz   10/98, 6/99, 7/99, 5/01, 8/01

Incremental cutting of a mathematically specified profile on the lathe

5/01:
Modified at the behest of Alfred Salzmann (Australia) to handle round tool
bits.

8/01:
Added self adjusting plot size.
...
#define NX 400			/* maximum number of profile points */
int nx=25;			/* actual number of profile points */
real scaler;			/* pr will be scaled by scaler */
real scalex;			/* px will be scaled by scalex */
/* px is measured (positive value) from end of stock (x=0) */
real px[NX];			/* profile x positions (in) */
real pr[NX];			/* profile radii (in) */
int nseg;			/* number of spline segments */
int seg=-1;			/* index of current spline segment */

#define NS 20			/* maximum number of spline segments */
int segs[NS];			/* spline segment start index */
int segf[NS];			/* spline segment finish index */
char fname[]="PROFILE.OUT";	/* output file */
char dfile[]="PROFILE.DAT";	/* data file */

#define NP 200			/* maximum number of spline points */
...
for (j=0 ; j<nseg ; j++)
{
if (x >= px[segs[j]] && x <= px[segf[j]])
	{
	if (seg != j)
		{
		for (i=segs[j],nspline=0 ; i<=segf[j] ; i++)
			{
			xsp[i-segs[j]]=px[i];
			ysp[i-segs[j]]=pr[i];
			nspline++;
...
/* if not in spline fit area, interpolate linearly */
for (i=0 ; i<nx ; i++) if (x <= px[i]) {k=i-1; break;}
return lin (px[k],pr[k],px[k+1],pr[k+1],x);
}
/* rdata..----------------------------------------------------------------- */
...
nx=0;			/* number of data entries */
nseg=0;			/* number of spline segments */
f=0;
while ((fgets(temp,sizeof(temp),fp)) != NULL)
	...
	if (temp[0] != '\0' && temp[0] != ';')
		...
		if (strstr(temp,"scalex"))
			{
			p=strtok(temp,"=\t;");
			scalex=(real)atof(strtok(NULL,"\t;"));
			continue;
			}
		if (strstr(temp,"scaler"))
			{
			p=strtok(temp,"=\t;");
		...
		if (strstr(temp,"sseg"))
			{
			p=strtok(temp,"=\t;");
			segs[nseg]=(int)atof(strtok(NULL,",\t;"));
			segf[nseg]=(int)atof(strtok(NULL,"\t;"));
			#if 0 == 1
			vpr ("nseg",&nseg,"d\n");
			vpr ("segs",&segs[nseg],"d\n");
			vpr ("segf",&segf[nseg],"d\n");
			#endif
			if (segf[nseg]-segs[nseg]+1 > NP)
			{
			printf
			("More than maximum (%d) points in spline segment\n",NP);
			quit;
			}
			nseg++;
			if (nseg == NS)
			{
			printf
			("More than maximum (%d) spline segments\n",NS);
			quit;
			}
			continue;
			}
		...
		if (strstr(temp,","))
		{
		px[nx]=atof(strtok(temp,",\t;"));
		pr[nx]=atof(strtok(NULL,"\t;"));
		if (++nx == NX)
			{
			printf
...
do		/* sort data */
{
for (i=0,f=0 ; i<nx-1 ; i++)
{
if (px[i] > px[i+1])
	{
	f=1;
	swap (px[i],px[i+1],sizeof(px[i]));
	swap (pr[i],pr[i+1],sizeof(pr[i]));
	}
}
} while (f);
...
for (i=0 ; i<nx ; i++)
{
px[i]*=scalex;		/* x position scaling */
pr[i]*=scaler;		/* radius scaling */
}
/* note that x=0 corresponds to the end of the stock */
xmin=-px[nx-1]; xmax=1.1*wt; ymax=2*ds; ymin=-ymax;
...
/* draw the profile */
for (x=-dxd,xl=-px[0],rl=pr[0] ; x>=-px[nx-1] ; x-=dxd)
{r=findr(-x); wline (xl,rl,x,r,15); wline (xl,-rl,x,-r,15); xl=x; rl=r;}

/* mark the data points for easy reference */
for (i=0 ; i<nx ; i++)
{
tic ('v',-px[i],rs,.005*((i % 10) ? 1 : 2.5),2);
tic ('v',-px[i],0,.005*((i % 10) ? 1 : 2.5),2);
}
...
for (xl=0,i=1 ; xl>=-px[nx-1] ; xl-=dxd,i++)
{
