/* LOFT.C   M. W. Klotz   1/05

Minimum length of thread engagement to guarantee that screw shears before
threads strip
...
char u;			//metric/Imperial flag
dbl D;			//basic thread diameter
dbl p;			//pitch
dbl tpi;		//threads per inch
dbl le;			//length of engagement
dbl dp;			//pitch circle diameter of thread
dbl ass;		//thread shear area
dbl at;			//screw thread tensile area
char ul[3];
char ua[5];
dbl x;
...
printf ("MINIMUM LENGTH OF THREAD ENGAGEMENT\n");
printf ("TO ENSURE SCREW BREAKS BEFORE THREADS STRIP\n\n");

u=getkey("(m)etric or [i]mperial ? ");
if (u != 'm') u='i';

if (u == 'i')
{strcpy (ul,"in"); strcpy (ua,"in^2");}
else
{strcpy (ul,"mm"); strcpy (ua,"mm^2");}

if (u == 'i') x=0.25; else x=4.;
vin ("Basic diameter of screw",&D,"lf",x,ul);

if (u == 'i')
{vin ("Pitch of screw",&tpi,"lf",20.,"tpi"); p=1/tpi;}
else
{vin ("Pitch of screw",&p,"lf",0.7,"mm");}

dp=D-0.64952*p;
at=0.25*PI*SQR(D-0.938194*p);
ass=0.5*PI*dp;
le=2.*at/ass;

print;
printf ("Pitch circle diameter of thread = %.4lf %s\n",dp,ua);
printf ("Screw thread tensile area = %.4lf %s\n",at,ua);
printf ("Thread shear area = %.4lf %s\n",ass,ua);
print;
printf ("Length of thread engagement = %.4lf %s\n",le,ul);
printf ("  or %.4lf threads\n",le/p);

printf ("\nNote:\n");
printf ("If J=(tensile strength of screw material)/(tensile strength of hole material)\n");
printf ("is greater than unity, multiply engagement length by J.\n");
printf ("[Calculations assume screw and hole are made of same material.]\n");
