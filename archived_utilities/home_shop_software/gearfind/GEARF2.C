/* GEARF2.C   M. W. Klotz   10/03, 12/04

Find two pairs of gears needed to create an a priori known ratio
...
puts ("FIND TWO PAIRS OF GEARS TO PRODUCE A GIVEN RATIO\n");
...
fp=modf(appr,&fint);
ffrac (fp,0.01*aerr);
ln=ln+fint*ld; g=gcd(ln,ld); ln/=g; ld/=g; z=(dbl)ln/(dbl)ld;
ar=ijr*z;

if (ln < gmin || ln > gmax) continue;
if (ld < gmin || ld > gmax) continue;
err=100.*(ar-dr)/dr;
if (ABS(err) > aerr) continue;
printf ("%d,",++nsol);
fprintf (ffp,"%d:%d - %ld:%ld => %.6lf (%.6lf%%)",i,j,ln,ld,ar,err);
