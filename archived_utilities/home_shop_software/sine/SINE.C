/* SINE.C   M. W. Klotz  11/98

Making sine bars with accurately spaced cylinders
...
{
int i,j,k;
dbl d1,d2,r,ang,s,q,dadd1,dadr;

/* -------------------------- initializations ------------------------------ */
textscreen ();
/* ------------------------------------------------------------------------- */

puts ("SINEBARS MADE WITH ACCURATELY SPACED CYLINDERS\n");
vin ("Diameter of first cylinder (d1)",&d1,"lf",0.375,"in");
vin ("Distance between cylinder centers (r)",&r,"lf",3.0,"in");
vin ("Desired angle",&ang,"lf",10.0,"deg");
d2=2.0*r*SIND(.5*ang)+d1;
if (.5*(d1+d2) <= r)
{
printf ("\nDIAMETER OF SECOND CYLINDER (d2) = %.4lf in\n\n",d2);
s=.5*(d2-d1)/r; q=1./sqrt(1.-s*s); dadd1=.5*q/r; dadr=q*ABS(.5*(d2-d1)/(r*r));
printf ("Angle error for d1 error of 0.001 in = %.4lf deg\n",0.001*DPR*dadd1);
printf ("Angle error for  r error of 0.001 in = %.4lf deg\n",0.001*DPR*dadr);
}
else
{
printf ("\nSecond cylinder too large to fit with r = %.4lf in\n",r);
puts ("Either use a smaller 'first' cylinder or larger r");
}
/* post run processing */
mexit:
exit(0);			/* normal exit */
