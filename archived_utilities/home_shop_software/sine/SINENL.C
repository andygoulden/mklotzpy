/* SINENL.C   M. W. Klotz  5/01

Making sine bars with accurately spaced cylinders
This version eliminates the link and uses just two touching cylinders.
...
puts ("SINEBARS MADE WITH TWO CYLINDERS\n");
vin ("Diameter of larger cylinder",&d2,"lf",0.75,"in");
vin ("Desired angle",&ang,"lf",1.5,"deg");
sa=SIND(.5*ang); d1=d2*(1.-sa)/(1+sa);
printf ("\nDIAMETER OF SMALLER CYLINDER = %.4lf in\n\n",d1);
/* post run processing */
mexit:
exit(0);			/* normal exit */
}
