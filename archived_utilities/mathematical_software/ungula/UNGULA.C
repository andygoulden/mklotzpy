/* UNGULA.C   M. W. Klotz   6/04

Compute volume of cylinder cut by a plane at an angle to the cylinder axis.
(The shape is called an ungula from its similarity to a cow's hoof.)
...
dbl ungula (dbl h, dbl s, dbl r);

/* gmark..-------------- global graphics variables ------------------------ */
/* some are contained in graphics.h */

/* vmark..------------ global variable declarations ----------------------- */
dbl dcyl;		// cylinder diameter
dbl rcyl;		// cylinder radius
dbl H;			// height of fluid above cylinder base
dbl sagitta;		// sagitta of ungula base
...
puts ("\nCOMPUTE VOLUME OF AN UNGULA\n");

vin ("Cylinder diameter",&dcyl,"lf",2.,"");
rcyl=0.5*dcyl;
vin ("Height of ungula",&H,"lf",10.,"");
agin:
vin ("Sagitta of ungula base",&sagitta,"lf",1.0,"");
if (sagitta > dcyl || sagitta <= 0.)
{
beep;
puts ("\n0 < sagitta <= diameter of cylinder is required");
goto agin;
}

printf ("\nVolume of ungula = %.4lf\n",ungula(H,sagitta,rcyl));
...
/* 
volume of ungula shape of height h and sagitta s and cylinder radius r
*/
...
b=s; a=sqrt(2.*b*r-b*b);
phi=0.5*PI+atan((b-r)/a); sp=sin(phi); cp=cos(phi);
return h*r*r*((3.*sp-3.*phi*cp-sp*sp*sp)/(1.-cp))/3.;
}
/* ------------------------------------------------------------------------ */
