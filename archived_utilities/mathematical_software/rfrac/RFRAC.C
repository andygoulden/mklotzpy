/* RFRAC.C   M. W. Klotz   11/98, 8,9/03

Rational approximation to decimal fractions using continued fractions
continued fractions are represented according to:
...
puts ("RATIONAL FRACTION APPROXIMATION\n");

if (argc == 1)
{
vin ("Number to approximate",&f,"lf",3.14159,""); f=ABS(f);
}
else f=ABS(atof(argv[1]));

gp: vin ("Desired approximation accuracy",&eps,"lf",0.01,"%");
if (eps < 0. || eps > 100.0) {beep; goto gp;}
eps*=0.01; fp=modf(f,&fint);

ffrac (fp,eps);		//form the continued fraction to the desired accuracy

g=gcd(ln,ld); ln/=g; ld/=g; z=(dbl)ln/(dbl)ld; app=fint+z;
print;
vpr ("Number",&f,"lf\n");
printf ("%ld & %ld/%ld = %ld/%ld = ",\
  (long)fint,ln,ld,ld*(long)fint+ln,ld); vpr ("",&app,"lf\n");
printf ("Approximation Error = %lg %%\n",100.*(z-fp)/fp);
